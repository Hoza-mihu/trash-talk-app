...
print("=" * 60)
print("Testing Backend with Hugging Face")
print("=" * 60)

try:
    # Don't pass provider - let it read from .env
    ...
    if predictor.provider:
        provider_name = type(predictor.provider).__name__
        if 'HuggingFace' in provider_name:
            print(f"\n✅ Using Hugging Face provider - FREE and ready!")
        else:
            print(f"\n⚠️  Using {provider_name} (not Hugging Face)")
